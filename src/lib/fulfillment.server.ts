...
export async function processOrderFulfillment(orderId: string) {
  ...
  for (const item of (order.order_items || [])) {
    ...
    if (!fulfillment) {
      const { data: created, error: createError } = await supabaseAdmin
        ...
        .insert({
          order_id: orderId,
          status: 'pending',
          fulfillment_type: product.delivery_method || 'manual',
    ...
    try {
      ...
      } else if (product.delivery_method === 'digital_file') {
      ...
      // In a real app, trigger email here
  ...
  // Check if all items are completed to update order status if needed
  // ...
}
...
async function deliverLicense(fulfillment: any, item: any, product: any, userId: string) {
  // Use existing claim_license RPC
...
async function deliverDigitalFile(fulfillment: any, item: any, product: any, userId: string) {
  // Verify file exists in metadata or media (assuming it's set up)
  // For now, we'll create an entitlement that grants access to the product files
  await supabaseAdmin.from('entitlements').insert({
    user_id: userId,
    product_id: product.id,
    order_id: item.order_id,
    ...
    data: {
      instructions: product.delivery_instructions,
      // In a real app, link to specific bucket objects
