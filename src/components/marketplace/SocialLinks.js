...
export const SocialLinks = ({ className, iconOnly = false, showLabels = true }) => {
    ...
    if (iconOnly) {
        ...
        {links.map((link) => (<a key={link.name} href={link.url} target="_blank" rel="noopener noreferrer" aria-label={link.aria} className={cn("text-muted-foreground transition-colors", link.color)}>
            <link.icon className="h-5 w-5"/>
          </a>))}
      </div>);
    }
    return (<div className={cn("flex flex-col gap-3", className)}>
      {links.map((link) => (<a key={link.name} href={link.url} target="_blank" rel="noopener noreferrer" aria-label={link.aria} className={cn("flex items-center gap-3 text-sm text-muted-foreground transition-colors group", link.color)}>
          ...
          {showLabels && (<div className="flex flex-col">
